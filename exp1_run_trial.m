function [data] = exp1_run_trial(win, chosenScreen, one_deg2Pix, data, iblock, trial_index, wave, contrast, ...
...
if s_interval == 1
    ...
    [g2] = create_grating(wave, 0, cpp, xp, duty, phase);
else
    [g1] = create_grating(wave, 0, cpp, xp, duty, phase); 
...
% if s_interval == 1   
% %     figure;
% %     plot(xp * cpp, masked_grating1, 'blue', 'LineWidth', 2);
% %     grid off;  
%     centerRow = round(size(masked_grating1, 1) / 2);
%     luminanceRange = masked_grating1(centerRow, :);   
%     pos = 1:length(luminanceRange); 
%     figure;        
%     plot(pos, luminanceRange, 'blue', 'LineWidth', 2);       
%     grid on;
% else
% % 
% %     figure;        
% %     plot(xp * cpp, masked_grating2, 'blue', 'LineWidth', 2);       
% %     grid on;
%     centerRow = round(size(masked_grating2, 1) / 2);
%     luminanceRange = masked_grating2(centerRow, :);   
%     pos = 1:length(luminanceRange); 
%     figure;        
%     plot(pos, luminanceRange, 'blue', 'LineWidth', 2);       
%     grid on;
